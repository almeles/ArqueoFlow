 ...
 * Monto Esperado = Monto Planilla - Total Devoluciones
 * @param {number} montoPlanilla
 * @param {number[]} devoluciones  Array of return amounts, e.g. [500, 200, 100]
 * @returns {number}
 */
function calcMontoEsperado(montoPlanilla, devoluciones) {
  const totalDevoluciones = devoluciones.reduce((sum, monto) => sum + monto, 0);
 ...
 * @param {number[]} session.devoluciones  Array of return amounts
...
function buildReconciliationMessage(session) {
  ...
  msg += `🛣️ Ruta: ${ruta}\n\n`;
  msg += `💰 Monto Planilla: ${formatCurrency(montoPlanilla)}\n`;

  if (devoluciones.length > 0) {
    msg += `(-) Devoluciones (${devoluciones.length}): ${formatCurrency(totalDevoluciones)}\n`;
  }

  msg += `(=) A Entregar (Neto): ${formatCurrency(montoEsperado)}\n`;
