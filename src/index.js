 ...
 *   AWAITING_DEVOLUCION_FACTURA - waiting for return invoice ID
 *   AWAITING_DEVOLUCION_MONTO   - waiting for return amount
...
async function sendArqueoMenu(ctx, session) {
  const totalDevoluciones = session.devoluciones.reduce((sum, d) => sum + d.monto, 0);
  ...
  if (session.devoluciones.length > 0) {
    ...
    session.devoluciones.forEach((d) => {
      msg += `  • Factura #${d.facturaId}: ${formatCurrency(d.monto)}\n`;
    });
    msg += `Total Devoluciones: ${formatCurrency(totalDevoluciones)}\n`;
  }

  msg += `\n*A Entregar: ${formatCurrency(montoEsperado)}*\n\n`;
...
bot.on('text', async (ctx) => {
  ...
  switch (session.state) {
    ...
    // -----------------------------------------------------------------------
    case 'ARQUEO_MENU': {
      if (text === '🔄 Agregar Devolución') {
        session.state = 'AWAITING_DEVOLUCION_FACTURA';
        await ctx.reply(
          '🔄 *Agregar Devolución*\n\nIngresa el *ID de Factura*:',
          { parse_mode: 'Markdown', reply_markup: removeKeyboard() }
        );
    ...
    // -----------------------------------------------------------------------
    case 'AWAITING_DEVOLUCION_FACTURA': {
      if (!text) {
        await ctx.reply('⚠️ Ingresa un ID de factura válido.');
        return;
      }
      session.currentDevolucionFacturaId = text;
      session.state = 'AWAITING_DEVOLUCION_MONTO';

      await ctx.reply(
        `✅ Factura: *#${text}*\n\nIngresa el *Monto de la Devolución* (en C$):\n_Ejemplo: 500_`,
        { parse_mode: 'Markdown' }
      );
      break;
    }
    ...
    case 'AWAITING_DEVOLUCION_MONTO': {
      ...
      const devolucion = {
        facturaId: session.currentDevolucionFacturaId,
        monto: amount,
      };
      session.devoluciones.push(devolucion);
      session.currentDevolucionFacturaId = null;

      await ctx.reply(
        `✅ Devolución registrada:\n  Factura: #${devolucion.facturaId}\n  Monto: ${formatCurrency(amount)}`
      );

      // Return to arqueo menu to allow adding more or proceeding
      await sendArqueoMenu(ctx, session);
      break;
    }
